...
flag_list = { -- generic inoffensive single color flags
	...
	"nazbol",
	"trihard7",
	"weed",  "youtube", "poggers", "covid", "get_off_my_property",
	"mlg",
	"dontreadonmemes", "anti_furry_army", "wolfenstein", "reddit",
	"rick_astley",
	"south_park", "soviet_america", "trollface", "big_chungus", "half_life",
	"deez_nuts",
	"anonpirate"}
end
...
local delete_if_orphan = function( self )
	...
	if node.name ~= "fictional_flags:upper_mast" and node.name ~= "ignore" then
...
minetest.register_entity( "fictional_flags:wavingflag", {
	initial_properties = {
		physical = false,
		visual = "mesh",
	...
	on_activate = function ( self, staticdata, dtime )
		...
		for o=1, #objs do
			...
			if lua and self ~= lua and lua.name == "fictional_flags:wavingflag" then
...
minetest.register_node("fictional_flags:lower_mast", {
...
local function spawn_flag( pos )
	...
	local obj = minetest.add_entity( flag_pos, "fictional_flags:wavingflag" )
...
minetest.register_node( "fictional_flags:upper_mast", {
	...
	paramtype2 = "facedir",
	tiles = { "flags_baremetal.png", "flags_baremetal.png" },
	wield_image = "ficflags_pole_top_inv.png",
	inventory_image = "ficflags_pole_top_inv.png",
	groups = { cracky = 2 },
	sounds = metal_sounds,
	is_ground_content = false,

	drop = {"fictional_flags:lower_mast 2", "fictional_flags:upper_mast"},
	...
	on_place = function( itemstack, placer, pointed_thing )
		...
		if (node.name ~= "fictional_flags:lower_mast" or node2.name ~= "fictional_flags:lower_mast") then
			return itemstack
		end
		...
		minetest.set_node(pointed_thing.under - vector.new(0, 1, 0), {name = "fictional_flags:upper_mast", param2 = param2 })
		minetest.set_node(pointed_thing.under, {name = "fictional_flags:upper_mast_hidden_1"})

		if not (minetest.is_creative_enabled(pn)) then
			itemstack:take_item()
		end

		local def = minetest.registered_nodes["fictional_flags:upper_mast"]
	...
	after_destruct = function( pos )
		...
		if minetest.get_node( above1 ).name == "fictional_flags:upper_mast_hidden_1" then
...
minetest.register_lbm({
	name = "fictional_flags:respawn_flags",
	label = "Respawn flags",
	nodenames = {"fictional_flags:upper_mast"},
...
minetest.register_lbm({
	name = "fictional_flags:update_node_meta",
...
if minetest.registered_items[flagpole_material] then
	minetest.register_craft({
		output = "fictional_flags:lower_mast 3",
		recipe = {
			{flagpole_material},
			{flagpole_material},
			{flagpole_material},
...
minetest.register_craft({
	output = "fictional_flags:upper_mast",
	recipe = {
...
-- or destructed.
minetest.register_node( "fictional_flags:upper_mast_hidden_1", {
	drawtype = "airlike",
	pointable = false,
	paramtype = "light",
...
flags.set_flag_at = function( pos, flag_name )
	...
	if node.name ~= "fictional_flags:upper_mast" then
		return false
	end
